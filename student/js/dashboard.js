 ...
 * Dashboard Portal Siswa — jadwal, kehadiran diri, observasi.
 */
...
async function init() {
    ...
    const STATUS_HDR_LABEL = { AKTIF: 'Aktif' };
    const STATUS_HDR_CLASS = { AKTIF: 'badge-status-aktif' };
...
// ─── Tab navigation ──────────────────────────────────────────
const TAB_SHORT = { jadwal: 'Jadwal', kehadiran: 'Hadir', observasi: 'Catatan' };
const TAB_ICON  = { jadwal: 'ti-calendar', kehadiran: 'ti-clipboard-check', observasi: 'ti-notes', forum: 'ti-messages' };

function buildTabs() {
    const nav    = document.getElementById('tab-nav');
    const botNav = document.getElementById('bottom-nav');
    const tabs   = [];
    tabs.push({ key: 'jadwal', label: 'Jadwal' });
