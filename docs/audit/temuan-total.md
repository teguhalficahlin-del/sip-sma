...
## Pembaruan 3 Juli 2026 — Audit Multi-Tenant (RPC Exposure)

Audit adversarial baru berbasis **introspeksi DB live** (bukan telaah statik) menemukan **kelas bug tersendiri di lapisan RPC**: fungsi `SECURITY DEFINER` generasi Juni–Juli 2026 mewarisi `GRANT EXECUTE TO PUBLIC` default sehingga **dapat dipanggil anon langsung via PostgREST, melewati auth Edge Function**. Terbukti hidup (probe anon `P0004`/`P0001`).

**Semua ditutup & diverifikasi live** (probe anon kini `42501 permission denied`), plus guard-rail otomatis `tests/tenant-isolation.mjs`. Dokumen lengkap: **[`audit-multi-tenant.md`](audit-multi-tenant.md)**.

| # | Sev | Temuan | Status |
|---|---|---|---|
| MT-1 | 🔴 Critical | `fn_batalkan_tahun_ajaran` anon-executable (rollback tahun ajaran destruktif) | ✅ FIXED (mig 190000) |
| MT-2 | 🟠 High | `fn_sync_observation/case/journal` injeksi lintas tenant oleh anon | ✅ FIXED (mig 190000) |
| MT-3 | 🟠 High | `fn_apply_schedule_templates` manipulasi jadwal (tenant dari param) | ✅ FIXED (mig 190000) |
| MT-4 | 🟡 Med→Low | `provision-admin` edge legacy (admin non-namespaced) | ✅ Dihapus repo + live |
| MT-5 | 🟡 Med | `fn_sync_*` tak validasi `student_id` satu sekolah | ✅ FIXED (mig 200000) |
| MT-6 | 🟡 Med | `fn_deactivate_stale_staff` privilege escalation dalam-tenant | ✅ FIXED (mig 190000) |
| MT-7 | 🔵 Low | GRANT tabel luas ke anon/authenticated (default; aman krn RLS on) | Observasi (guard-rail CHECK 1) |
| MT-8 | 🔵 Low | `fn_stakeholder_summary` + `fn_update_school_branding` anon-exec | ✅ FIXED (mig 190000/210000) |

Sisi positif audit ini: **RLS 28/28 tabel + 103/103 policy filter tenant = bersih** (isolasi lapisan DB terbukti). Kredensial default `Admin/Admin1234` (SMK Harapan Rokan) **direset**. Commit `0a1b04f`, `bf78a83`, `ba15ffd`.
